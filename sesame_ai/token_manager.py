...
class TokenManager:
    ...
    def _is_token_expired(self, id_token):
        ...
        try:
            ...
            # Assume token might be invalid if we can't check
            return True
    ...
    def get_valid_token(self, force_new=False):
        ...
        if id_token:
            # Check if the token is still valid - quietly catch errors
            try:
                ...
                logger.warning(f"Token validity check failed: {e}")
                # If we can't reliably check, create a new token
                return self._create_new_account()
            ...
            if refresh_token:
                try:
                    ...
                    logger.error(f"Token refresh failed, creating new: {e}")
                    return self._create_new_account()
            else:
