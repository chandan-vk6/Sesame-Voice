...
class VoiceChat:
    ...
    def setup_audio_streams(self):
        ...
        logger.debug("Setting up audio streams")
         # Log device info before opening streams
        if self.output_device_index is not None:
            output_info = self.p.get_device_info_by_index(self.output_device_index)
            logger.debug(f"Using output device: {output_info['name']} with rate {self.output_rate}")
        ...
        while self.running:
            try:
                ...
                if audio_chunk:
                    ...
                    chunk_counter += 1
                    if chunk_counter % 100 == 0:  # Don't spam logs
                        logger.debug(f"Playing audio chunk #{chunk_counter}, size: {len(audio_chunk)} bytes")
                    # Play audio immediately when received
                    self.output_stream.write(audio_chunk)
                else:
                    if chunk_counter > 0 and chunk_counter % 500 == 0:  # Check occasionally
                        logger.debug("No audio chunks received recently")
            except Exception as e:
                if self.running:
